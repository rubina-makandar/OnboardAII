...
--- USER
-- Drop tables if already exist (to avoid duplicate errors)
...
('Rehan Ahmed', 'rehan', 'hashed_password1', 'admin'),
('Ayesha Khan', 'ayesha', 'hashed_password2', 'employee'),
('Ravi Kumar', 'ravi', 'hashed_password3', 'employee'),
('Sneha Sharma', 'sneha', 'hashed_password4', 'employee');

-- =========================
-- Insert Sample Tasks
-- =========================
INSERT INTO tasks (title, description, assigned_to, status) VALUES
('Prepare Monthly Report', 'Generate and submit the financial report for August.', 2, 'in_progress'),
('Update Website', 'Fix the navigation bug and update the About Us page.', 3, 'pending'),
('Client Meeting', 'Schedule and attend the meeting with XYZ client.', 4, 'completed'),
('Database Backup', 'Take a full backup of the system database.', 2, 'pending');

-- =========================
-- Insert Sample Notifications
-- =========================
INSERT INTO notifications (message, recipient, type, date, is_read) VALUES
('New task assigned: Prepare Monthly Report', 2, 'task', CURDATE(), FALSE),
('Reminder: Update Website is still pending', 3, 'reminder', CURDATE(), FALSE),
('Task completed: Client Meeting', 4, 'task', CURDATE(), TRUE),
('System Alert: Database backup scheduled', 2, 'alert', CURDATE(), FALSE);

-- =========================
-- Example Queries
-- =========================

-- 1. Show all tasks with assigned user name
SELECT t.id, t.title, t.status, u.full_name AS assigned_user
FROM tasks t
LEFT JOIN users u ON t.assigned_to = u.id;

-- 2. Show all unread notifications with recipient name
SELECT n.id, n.message, n.type, n.date, u.full_name AS recipient_name
FROM notifications n
JOIN users u ON n.recipient = u.id
